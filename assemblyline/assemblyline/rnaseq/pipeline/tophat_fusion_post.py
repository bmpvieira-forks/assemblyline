...
def map_fusion_kmer(bwt_idx_prefix, params, fusion_input_file):
    def get_fusion_seq():
        seq_dic = {}

        fusion_file = open(fusion_input_file, 'r')
...
def read_dist(params, bam_file_name):
    def check_region(sample_name, bam, check_list):
        ...
        reads_compress_list = [[1, 0] for i in range(len(check_list))]
            ...
            for i in range(len(check_list)):
                ...
                if (fusion_read and chr1 == chr1_ref and chr2 == chr2_ref) or \
                    ...
                    if fusion_read:
                        ...
                        # daehwan - remove this
                        if saw_fusion and False:
                            if abs(pos1 - pos1_ref) < 3 and abs(pos2 - pos2_ref) < 3:
                                print chr1_ref, chr2_ref, left_pos, right_pos, cigars, mismatch, params.fusion_read_mismatches
                                print pos1, pos1_ref
                                print pos2, pos2_ref
                                sys.exit(1)
    ...
    for sample_name, list in check_list.items():
        ...
        for i in range((len(list) + increment - 1) / increment):
            ...
            def work():
                if len(check_list) > 0:
                    check_region(sample_name, bam_file_name, check_list)
...
def generate_html(params):
            ...
            if line_no % 6 == 0:
                ...
                if not do_not_add and os.path.exists(read_output_file):
                    ...
                    if len(read_output) < 1024 * 1024 * 1024:
                        ...
                        output["score"] = score
                        output["test"] = "lcount: %d, lavg: %d, lgap: %d, lder: %f, rcount: %d, ravg: %d, rgap: %f, rder: %f, rate: %f, dist: %d, score: %f" % \
    ...
    def generate_html_impl(fusion_list, cluster_list):
        ...
        print >> html_file, r'Known fusions are found at <a href="#mcl">Mitelman Database</a> as of Feb 21, 2012<P>'
...
def main(argv=None):
    ...
    try:
        ...
        fusion_file = input_dir + "/fusions.out"
        ...
        if not params.skip_fusion_kmer:
            map_fusion_kmer(bwt_idx_prefix, params, fusion_file)

        if not params.skip_filter_fusion:
            filter_fusion(bwt_idx_prefix, refgene_file, ensgene_file, params, fusion_file, sample_name)

        if not params.skip_blast:
            do_blast(params)

        if not params.skip_read_dist:
            read_dist(params, bam_file)
