...
def traceback(G, sink, inclusive=False):
    """
...
def find_seeded_path(G, seed, source, sink):
    # dynamic programming search for best path
    dynprog_search(G, source)
    path1, weight1, length1 = traceback2(G, seed)
    # dynamic programming search for best path
    dynprog_search(G, seed)
    path2, weight2, length2 = traceback2(G, sink)
...
def find_suboptimal_paths(G, source, sink, 
    ...
    while (len(used_nodes) > 0) and (iterations < max_paths):
        ...
        used_nodes.difference_update(path)
        iterations +=1
