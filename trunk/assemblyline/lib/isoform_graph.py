    ...
    for i in xrange(num_leaf_exons):
        ...
        for j in xrange(i+1, len(exon_tuples)):
            ...
            if (len(e1_tuple.exon_domains.intersection(e2_tuple.intron_domains)) == 0 and
                len(e2_tuple.exon_domains.intersection(e1_tuple.intron_domains)) == 0 and
                interval_overlap_threshold(e1_tuple.node, e2_tuple.node, 1)):
                Gmerge.add_edge(i,j)
                #print 'compatible', i, j, e1_tuple.node, e2_tuple.node
    return Gmerge

def make_exon_merge_tuple(G, strand, exon_tuples):
    preds = set()
    succs = set()
    ids = set()
    strand = NO_STRAND
...
def merge_exons(G, strand, exon_tuples, k):
    ...
    for clique_indexes in kcliques(Gmerge, k):
        if len(clique_indexes) < 2:
            continue
        clique_exon_tuples = [exon_tuples[i] for i in clique_indexes]
        # merge the exons into a new node
        merge_tuple = make_exon_merge_tuple(G, strand, clique_exon_tuples)
        if len(clique_exon_tuples) > 2:
