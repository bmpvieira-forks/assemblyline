...
def traceback(G, sink):
    ...
    """
    # initialize path to start with sink node
    path = [sink]
...
def find_path(G, source, sink, reverse=False):
    if reverse:
    ...
    path, weight, length = traceback(G, sink)
    # restore edges
    if reverse:
        source, sink = sink, source
        reverse_graph(G)
...
def find_seeded_path(G, source, seed, sink):
    # dynamic programming search for best path
    dynprog_search(G, source)
    # traceback to get path
    path, weight, length = traceback(G, sink)
